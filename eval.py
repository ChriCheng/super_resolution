...
def parse_args():
    parser = argparse.ArgumentParser(description="Evaluate ESPCN x4 on Set5 with MindSpore")
    parser.add_argument("--set5_hr_dir", type=str, required=True, help="Path to Set5 HR images")
...
def main():
    ...
    loader = create_eval_loader(args.set5_hr_dir, scale=4)
    rows = []
    ...
    for batch in loader.create_tuple_iterator(num_epochs=1):
        ...
        psnr, ssim = calc_psnr_ssim(sr_uint8, hr_uint8)

        img_name = name.asnumpy()[0].decode("utf-8") if hasattr(name, "asnumpy") else str(name)
    ...
    with open(os.path.join(args.save_dir, "summary.json"), "w", encoding="utf-8") as f:
        json.dump({"average_psnr": avg_psnr, "average_ssim": avg_ssim}, f, indent=2, ensure_ascii=False)

    print("=" * 60)
    print(f"Set5 x4 Average PSNR: {avg_psnr:.4f} dB")
    print(f"Set5 x4 Average SSIM: {avg_ssim:.6f}")
